...
int timer_gettime(timer_t timerid, struct itimerspec *value)
{
    ...
    if (os_timer_stat(&ptimer->timer)) {
        value->it_value.tv_sec = (time_t)(expired_time / OS_HZ);
        value->it_value.tv_nsec = (long)((expired_time % OS_HZ) * NANOSECONDS_PER_TICK);
    } else {
    ...
    if (period_time != osWaitForever) {
        value->it_interval.tv_sec = (time_t)(period_time / OS_HZ);
        value->it_interval.tv_nsec = (long)((period_time % OS_HZ) * NANOSECONDS_PER_TICK);
    } else {
