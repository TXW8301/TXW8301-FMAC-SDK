...
int32 system_ap_sleep_io_wakeup_hook(void);
int32 system_ap_sleep_rxdata_hook(uint8 *data, uint32 len);
void system_ap_sleep_wakeup_hook(void);
void system_sleep_config(uint32 cmd, uint32 param1, uint32 param2);
void system_sleep_reset(void);
void dsleep_norflash_reboot(void);
void dsleep_back_to_dsleep(uint32 time_us);
