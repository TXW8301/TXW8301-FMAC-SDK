...
static int32 hggpio_v2_analog(struct gpio_device *gpio, uint32 pin, int32 value)
{
    struct hggpio_v2 *dev = (struct hggpio_v2 *)gpio;
...
static int32 hggpio_v2_input_delay_en(struct gpio_device *gpio, uint32 pin, int32 value)
{
    struct hggpio_v2 *dev = (struct hggpio_v2 *)gpio;
...
static int32 hggpio_v2_ioctl(struct gpio_device *gpio, uint32 pin, int32 cmd, int32 param1, int32 param2)
{
    int32 ret_val = RET_OK;

    switch (cmd) {
        case GPIO_INPUT_DELAY_ON_OFF:
            ret_val = hggpio_v2_input_delay_en(gpio, pin, param1);
            break;
        case GPIO_VALUE_ATOMIC:
            ret_val = hggpio_v2_set_atomic(gpio, pin, param1);
            break;
        case GPIO_DIR_ATOMIC:
            ret_val = hggpio_v2_dir_atomic(gpio, pin, param1);
            break;
        case GPIO_CMD_DRIVER_STRENGTH:
            ret_val = hggpio_v2_driver_strength(gpio, pin, param1);
            break;
        case GPIO_CMD_AFIO_SET:
            ret_val = hggpio_v2_afio(gpio, pin, param1);
            break;
        case GPIO_GENERAL_ANALOG:
            ret_val = hggpio_v2_analog(gpio, pin, param1);
            break;
        default:
            ret_val = -ENOTSUPP;
            break;
...
static const struct gpio_hal_ops gpio_v2_ops = {
    .mode            = hggpio_v2_mode,
    .dir             = hggpio_v2_dir,
    .set             = hggpio_v2_set,
    .get             = hggpio_v2_get,
    .ioctl           = hggpio_v2_ioctl,
    .request_pin_irq = hggpio_v2_request_pin_irq,
    .release_pin_irq = hggpio_v2_release_pin_irq,
};

__init int32 hggpio_v2_attach(uint32 dev_id, struct hggpio_v2 *gpio)
{
    int32 i = 0;
    gpio->dev.dev.ops = (const struct devobj_ops *)&gpio_v2_ops;
    for (i = 0; i < HGGPIO_V2_MAX_PINS; i++) {
        gpio->irq_hdl[i] = NULL;
    }
