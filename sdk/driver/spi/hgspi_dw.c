...
static int32 hgspi_dw_ioctl(struct spi_device *p_spi, uint32 cmd, uint32 param1, uint32 param2)
{
    struct hgspi_dw *dev = (struct hgspi_dw *)p_spi;
    uint32 temp;
...
//                        break;
        ...
        case SPI_SET_CS:
            hgspi_dw_set_cs(p_spi, param1, param2);
            break;
        case HGSPI_DW_INTR_SET:
            hgspi_dw_unmask_interrupt(dev->hw, param1);
            break;
        case HGSPI_DMAC_USE:
            dev->dma = (struct dma_device *)param1;
            if(dev->dma) {
                dev->hw->DMATDLR = 0;
                dev->hw->DMARDLR = 0;
...
static int32 hgspi_dw_request_irq(struct spi_device *p_spi, uint32 irq_flag, spi_irq_hdl irqhdl, uint32 irq_data)
{
    struct hgspi_dw *dev = (struct hgspi_dw *)p_spi;
...
static const struct spi_hal_ops spi_dw_ops = {
    .open         = hgspi_dw_open,
    .close        = hgspi_dw_close,
    .ioctl        = hgspi_dw_ioctl,
    .read         = hgspi_dw_read,
    .write        = hgspi_dw_write,
    .request_irq  = hgspi_dw_request_irq,
};
...
__init int32 hgspi_dw_attach(uint32 dev_id, struct hgspi_dw *p_spi)
{
    ...
    p_spi->irq_hdl          = NULL;
    p_spi->irq_data         = 0;
    p_spi->dev.dev.ops      = (const struct devobj_ops *)&spi_dw_ops;
