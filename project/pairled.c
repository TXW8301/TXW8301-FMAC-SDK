...
struct {
    uint8 conn: 1, rssi1: 1, rssi2: 1, rssi3: 1, val: 1, io1_mode: 1, io2_mode: 1, pair: 1;
    uint8 wk_delay;
} sys_ledctrl;
        ...
        case IEEE80211_EVENT_RSSI:
            if (ifidx == WIFI_MODE_STA && !sys_ledctrl.pair) {
                if ((int8)param1 > -48 && sys_ledctrl.conn) {
                    ...
                    sys_ledctrl.rssi1 = sys_ledctrl.conn;
                    sys_ledctrl.rssi2 = 0;
                    sys_ledctrl.rssi3 = 0;
                }
            }
...
{
    if (sys_ledctrl.wk_delay == 0) {
        sys_ledctrl.wk_delay = 100;
        sys_ledctrl.io1_mode = 1;
        sys_ledctrl.io2_mode = 1;
    }

    if (sys_ledctrl.conn || sys_ledctrl.rssi1) {
        if (sys_ledctrl.io1_mode) {
            gpio_set_dir(WNB_RSSI_IO1, GPIO_DIR_OUTPUT);
            sys_ledctrl.io1_mode = 0;
        }
        if ((sys_ledctrl.conn && sys_ledctrl.val == 0) || (sys_ledctrl.rssi1 && sys_ledctrl.val == 1)) {
            gpio_set_val(WNB_RSSI_IO1, sys_ledctrl.val);
        }
    } else {
        if (!sys_ledctrl.io1_mode) {
            sys_ledctrl.io1_mode = 1;
            gpio_set_dir(WNB_RSSI_IO1, GPIO_DIR_INPUT);
        }
    }

    if (sys_ledctrl.rssi2 || sys_ledctrl.rssi3) {
        if (sys_ledctrl.io2_mode) {
            gpio_set_dir(WNB_RSSI_IO2, GPIO_DIR_OUTPUT);
            sys_ledctrl.io2_mode = 0;
        }
        if ((sys_ledctrl.rssi2 && sys_ledctrl.val == 0) || (sys_ledctrl.rssi3 && sys_ledctrl.val == 1)) {
            gpio_set_val(WNB_RSSI_IO2, sys_ledctrl.val);
        }
    } else {
        if (!sys_ledctrl.io2_mode) {
            sys_ledctrl.io2_mode = 1;
            gpio_set_dir(WNB_RSSI_IO2, GPIO_DIR_INPUT);
        }
    }
